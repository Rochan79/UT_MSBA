...
Now to calculate range, mean, sd of all columns 
```{r , error=FALSE, warning=FALSE}
...
```
* all predictors except chas are statistically significant
* The coefficients show a positive linear relationship between crim and ptratio and a negative relationship between crim and dis
...
```
Best Lambda is 376.98
The MSE  obtained is 1711214
...
We will find out for what number of terminal nodes we get least error
```{r, echo=FALSE, error=FALSE, warning=FALSE}
set.seed(2)
...
```{r, echo=FALSE, error=FALSE, warning=FALSE}
set.seed(2)
rf1.carseats = randomForest(Sales~., data=Carseats.train, mtry=5, ntree=500, importance=T)
...
RF for m=10/2, sqrt(10), 20 yields the following MSE
```{r, echo=FALSE, error=FALSE, warning=FALSE}
set.seed(2)
...
## Part C
Predict that a person will make a purchase if the estimated probability of purchase is greater than 20 %, get confusion matrix and compare it with logistic regresssion
```{r, echo=FALSE, error=FALSE, warning=FALSE}
set.seed(2)
...
```
Yes there is a premium for Brick houses, keeping variables constant, one would give $17297 premium. 
## Part B
